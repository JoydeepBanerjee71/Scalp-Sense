...
async function loadDoctors(){
    ...
    getDocs(q)
        .then(snapshot=>{
            ...
            snapshot.docs.forEach(doc=>{
                const doctorUid = doc.data().uid;
                if(!doctorUid || loadedDoctorIds.has(doctorUid)){
                    return;
                }
                loadedDoctorIds.add(doctorUid);
                const img = doc.data().image || "https://via.placeholder.com/80x80?text=Dr";
                const name = doc.data().name || "ScalpSense Doctor";
                const experience = doc.data().experience || "Experience not shared";
                const location = doc.data().location || "Location not shared";
                let card = createContactCard(doctorUid, img, name, experience, location);
...
async function loadChatbotHistory(){
    ...
    document.getElementById('chat-name').innerText = "ScalpSense Chatbot";
    const userRef = doc(db,'Users',uid);
